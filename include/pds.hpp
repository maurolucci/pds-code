...
enum class PmuState : signed char { Blank = 0, Active = 1, Inactive = -1 };
...
using PrecedenceDigraph =
    mpgraphs::VecGraph<mpgraphs::Empty, mpgraphs::EdgeDirection::Bidirectional,
                       true, Timestamp>;
using Node = PrecedenceDigraph::VertexDescriptor;
...
template <typename T>
using EdgeMap =
    mpgraphs::VecMap<PowerGrid::VertexDescriptor, VertexMap<T>, Timestamp>;
#endif
using VertexList = std::vector<PowerGrid::VertexDescriptor>;
using EdgeList = std::vector<PrecedenceDigraph::EdgeDescriptor>;
...
public:
  ...
  [[nodiscard]] inline size_t numZeroInjection() const {
    return ranges::distance(
        graph().vertices() |
        ranges::views::filter([this](auto v) { return isZeroInjection(v); }));
  }
