...
size_t treeWidth(const PowerGrid &graph) {
  ...
  auto treeDecompositionAlgo =
      std::make_unique<htd::CombinedWidthMinimizingTreeDecompositionAlgorithm>(
          library.get());
...
VertexMap<size_t> propagationDistance(const PowerGrid &graph) {
  ...
  for (auto v : graph.vertices()) {
    if (step.contains(v) && unobservedDegree[v] == 1)
      queue.push_back(v);
  }
  for (auto v : graph.vertices()) {
    ...
    size_t unobserved = std::ranges::distance(
        neighbors |
        ranges::views::filter([&step](auto v) { return !step.contains(v); }));
    unused(unobserved);
...
void writeSolutionStatistics(const std::string_view &name,
  ...
  ssize_t maxStep = -1;
  for (auto v : step)
    maxStep = std::max(maxStep, ssize_t(v.second));
  fmt::print(
      out,
      ...
      "n_blank"_a =
          state.graph().numVertices() - state.numActive() - state.numInactive(),
      "n_observed"_a = state.numObserved(), "propagation_distance"_a = maxStep,
...
auto getSolver(po::variables_map &vm, FortStats &fortStats,
               FortStats &cycleStat, FortStats &pathStat, FortStats &efpsStat,
  ...
  bool intermediateEfpss = vm.count("intermediate-efpss");
  ...
  cyclecallback::CycleCallback cycleCallback =
      [&, solved = size_t{0}](cyclecallback::When when, const PdsState &state,
                              const std::vector<VertexList> &cycles,
                              size_t lower, size_t upper) mutable {
  ...
  pathcallback::PathCallback pathCallback =
      [&, solved = size_t{0}](pathcallback::When when, const PdsState &state,
                              const std::vector<VertexList> &paths,
                              size_t lower, size_t upper) mutable {
  ...
  efpscallback::EFPSCallback efpsCallback =
      [&, solved =
              size_t{0}](efpscallback::When when, const PdsState &state,
                         const std::vector<std::pair<EdgeList, size_t>> &efpss,
                         size_t lower, size_t upper) mutable {
        if (vm.count("fort-stats") && when == pds::efpscallback::When::FINAL) {
          size_t totalEfpsSize = 0;
          for (auto &efps : efpss) {
            totalEfpsSize += efps.first.size();
          }
          double averageSize = double(totalEfpsSize) / double(efpss.size());
          efpsStat = {averageSize, efpss.size(), solved};
        }
        ++solved;
      };
  if (solverName == "branching") {
    return Solver{[](auto &state, double) {
      return solveBranching(state, true, greedy_strategies::largestDegree);
    }};
    ...
    return Solver{[=](auto &state, double timeLimit) {
      ...
      return solveBozeman(state, verbose, timeLimit, 1, fortInit,
                          greedyBoundMode, earlyStop, fortCallback, boundCB,
                          -1);
    }};
    ...
    return Solver{[=](auto &state, double timeLimit) {
      return solveCycles(state, verbose, timeLimit, 0, cycleInit,
                         greedyBoundMode, earlyStop, cycleCallback, boundCB,
                         intermediateCycles ? 0 : -1, cycleLimit);
    }};
  } else if (solverName == "efpss") {
    return Solver{[=](auto &state, double timeLimit) {
      return solveEFPS(state, verbose, timeLimit, 0, efpsInit, greedyBoundMode,
                       earlyStop, efpsCallback, boundCB,
                       intermediateEfpss ? 0 : -1, efpsLimit);
    }};
    ...
    return Solver{[=](auto &state, double timeLimit) {
      return solveCyclesForts(state, verbose, timeLimit, 0, cycleInit,
                              ...
                              cycleCallback, boundCB, -1, cycleLimit);
    }};
  ...
  } else if (solverName == "cycles-forts2") {
    return Solver{[=](auto &state, double timeLimit) {
    ...
    return Solver{[=](auto &state, double timeLimit) {
      return solvePaths(state, verbose, timeLimit, 0, pathInit, greedyBoundMode,
                        earlyStop, pathCallback, boundCB, -1, pathLimit);
    }};
  } else if (solverName == "paths11") {
    return Solver{[=](auto &state, double timeLimit) {
      return solvePaths(state, verbose, timeLimit, 11, pathInit,
                        greedyBoundMode, earlyStop, pathCallback, boundCB, -1,
                        pathLimit);
    }};
  ...
  } else if (solverName == "paths1221") {
    return Solver{[=](auto &state, double timeLimit) {
      return solvePaths(state, verbose, timeLimit, 1221, pathInit,
                        greedyBoundMode, earlyStop, pathCallback, boundCB, -1,
                        pathLimit);
    }};
    ...
    return Solver{[=](auto &state, double timeLimit) {
      return solvePathsForts(state, verbose, timeLimit, 0, fortInit,
                             greedyBoundMode, earlyStop, fortCallback,
                             pathCallback, boundCB, -1, pathLimit);
    }};
  } else if (solverName == "paths-forts11") {
    return Solver{[=](auto &state, double timeLimit) {
      return solvePathsForts(state, verbose, timeLimit, 11, fortInit,
                             greedyBoundMode, earlyStop, fortCallback,
                             pathCallback, boundCB, -1, pathLimit);
    }};
  } else if (solverName == "paths-forts12") {
    return Solver{[=](auto &state, double timeLimit) {
      return solvePathsForts(state, verbose, timeLimit, 12, fortInit,
                             greedyBoundMode, earlyStop, fortCallback,
                             pathCallback, boundCB, -1, pathLimit);
    }};
  } else if (solverName == "paths-forts13") {
    return Solver{[=](auto &state, double timeLimit) {
      return solvePathsForts(state, verbose, timeLimit, 13, fortInit,
                             greedyBoundMode, earlyStop, fortCallback,
                             pathCallback, boundCB, -1, pathLimit);
    }};
  } else if (solverName == "paths-forts21") {
    return Solver{[=](auto &state, double timeLimit) {
      return solvePathsForts(state, verbose, timeLimit, 21, fortInit,
                             greedyBoundMode, earlyStop, fortCallback,
                             pathCallback, boundCB, -1, pathLimit);
    }};
  } else if (solverName == "paths-forts22") {
    return Solver{[=](auto &state, double timeLimit) {
      return solvePathsForts(state, verbose, timeLimit, 22, fortInit,
                             greedyBoundMode, earlyStop, fortCallback,
                             pathCallback, boundCB, -1, pathLimit);
    }};
  } else if (solverName == "paths-forts31") {
    return Solver{[=](auto &state, double timeLimit) {
      return solvePathsForts(state, verbose, timeLimit, 31, fortInit,
                             greedyBoundMode, earlyStop, fortCallback,
                             pathCallback, boundCB, -1, pathLimit);
    }};
  } else if (solverName == "paths-forts32") {
    return Solver{[=](auto &state, double timeLimit) {
      return solvePathsForts(state, verbose, timeLimit, 32, fortInit,
                             greedyBoundMode, earlyStop, fortCallback,
                             pathCallback, boundCB, -1, pathLimit);
    }};
  } else if (solverName == "paths-forts33") {
    return Solver{[=](auto &state, double timeLimit) {
      return solvePathsForts(state, verbose, timeLimit, 33, fortInit,
                             greedyBoundMode, earlyStop, fortCallback,
                             pathCallback, boundCB, -1, pathLimit);
    }};
  } else if (solverName == "paths-forts1221") {
    return Solver{[=](auto &state, double timeLimit) {
      return solvePathsForts(state, verbose, timeLimit, 1221, fortInit,
                             greedyBoundMode, earlyStop, fortCallback,
                             pathCallback, boundCB, -1, pathLimit);
    }};
  } else if (solverName == "paths-forts1321") {
    return Solver{[=](auto &state, double timeLimit) {
      return solvePathsForts(state, verbose, timeLimit, 1321, fortInit,
                             greedyBoundMode, earlyStop, fortCallback,
                             pathCallback, boundCB, -1, pathLimit);
    }};
  } else {
...
int main(int argc, const char **argv) {
  ...
  desc.add_options()("help,h", "show this help")(
      ...
      "greedy-bounds,b", po::value<int>()->default_value(0)->implicit_value(1),
      ...
      "cycle-init", po::value<int>()->implicit_value(0)->default_value(0),
  ...
  if (reductionName == "none") {
    ...
    fmt::print(stderr, "invalid reduction mode: {}. modes: {}", reductionName,
  ...
  if (vm.count("outfile")) {
    ...
    if (!outfile)
      throw std::ios_base::failure(
          strerror(errno), std::error_code(errno, std::system_category()));
  ...
  if (vm.count("stat-file")) {
    ...
    fmt::print(statFile, "# degrees format: "
                         "<#zi+inactive>:<#zi+blank>:<#zi+active>:<#nonzi+"
                         "inactive>:<#nonzi+blank>:<#nonzi.active>;...\n");
    fmt::print(statFile, "{}",
               "name,n,m,n_zero_injection,n_pmu,n_inactive,n_blank,n_observed,"
               "propagation_distance,tree_width,degrees\n");
  }
  ...
  for (auto out : outputs) {
    ...
    fmt::print(
        out, "{}\n",
        "name,run,bound,pmus,solved,result,t_total,t_reductions,t_solver,n,m,"
        "zi,n_reduced,m_reduced,zi_reduced,pmu_reduced,blank_reduced");
  }
  ...
  for (const std::string &filename : inputs) {
    ...
    for (size_t run = 0; run < repetitions; ++run) {
      ...
      if (vm.count("write-bounds")) {
        ...
        auto boundsFileName =
            fmt::format("{}/{}-{}-bounds.csv", boundsDir, currentName, run);
      ...
      if (state.allObserved()) {
        if (boundsFile) {
          const auto &b = BoundInfo{state.numActive(), state.numActive(), 0,
                                    µs(now() - t0)};
        ...
        ranges::sort(subproblems, [](const auto &left, const auto &right) {
        ...
        for (auto &substate : subproblems) {
          if (!substate.allObserved()) {
            ...
            auto boundCB = [&bounds, t0, result, boundsFile, &currentName,
                            &subproblemNumber, run, initialBlank,
                            initialActive](size_t lower, size_t upper,
                                           size_t extra) {
              auto time = now();
              ...
              if (boundsFile) {
                ...
                fmt::print(boundsFile, "{},{},{},{},{},{},{},{}\n", currentName,
                           run, subproblemNumber, b.time, b.lower, b.upper, gap,
                           b.extraInfo);
                fflush(boundsFile);
              }
            };
            Solver solve =
                getSolver(vm, fortStats, cycleStats, pathStats, efpsStats,
                          currentName, run, subproblemNumber, boundCB);
            auto tSubproblem = now();
            double remainingTimeout = std::max(
                1.0, timeout - std::chrono::duration_cast<std::chrono::seconds>(
        ...
        auto boundCB = [&bounds, t0, boundsFile, &currentName,
                        run](size_t lower, size_t upper, size_t extra) {
          auto time = now();
          ...
          if (boundsFile) {
            ...
            double gap = (double(b.upper) - double(b.lower)) / double(b.upper);
            fmt::print(boundsFile, "{},{},{},{},{},{},{},{}\n", currentName,
                       run, 0, b.time, b.lower, b.upper, gap, b.extraInfo);
            fflush(boundsFile);
          }
        };
        Solver solve = getSolver(vm, fortStats, cycleStats, pathStats,
                                 efpsStats, currentName, run, 0, boundCB);
      ...
      if (drawdir) {
        ...
        writePds(state.graph(), fmt::format("{}/2_solved_preprocessed.pds",
                                            drawdir->string()));
      }
      if (solDir) {
      ...
      for (auto file : outputs) {
        fmt::print(
            file,
            "{name},{run},{lower_bound},{pmus},{solved},{result},{t_total},{t_"
            "reductions},{t_solver},{n},{m},{zi},{nReduced},{mReduced},{"
            "ziReduced},{pmuReduced},{blankReduced}\n",
            "name"_a = filename, "run"_a = run, "lower_bound"_a = result.lower,
            "pmus"_a = pmus, "solved"_a = state.allObserved(),
            "result"_a = result.state, "t_total"_a = µs(t2 - t0),
            "t_reductions"_a = µs(t1 - t0), "t_solver"_a = µs(t2 - t1),
            "n"_a = n, "m"_a = m, "zi"_a = zi, "nReduced"_a = nReduced,
            "mReduced"_a = mReduced, "ziReduced"_a = ziReduced,
            "pmuReduced"_a = pmusReduced, "blankReduced"_a = blankReduced);
      }
      if (statFile != nullptr) {
