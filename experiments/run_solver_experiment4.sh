...
for OTHER_ARGS in ''; do
    ...
    date
    time ./experiment -s efpss -n $NRUN $OTHER_ARGS --timeout=$TIMEOUT -o "results/solver/efps$OTHER_ARGS.csv" --fort-stats results/solver/efps$OTHER_ARGS-fort.csv -w results/solver/efps$OTHER_ARGS/sol $(cat $CASES)
    date
done
